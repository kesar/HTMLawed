...
elseif(strpos($x, '*') === false){$e = array_flip(explode(',', $x));}
 ...
 if(isset($x[1])){
  if(strpos($x, '(')){
   $x = preg_replace_callback('`\([^()]+\)`', function($m){return str_replace(array('(', ')', '-'), array('', '', 'A'), $m[0]);}, $x);
  }
  ...
  foreach($m as $v){
   ...
   elseif($v[0] == '-'){
    if(strpos($v, '-', 1)){$e[$v] = 1;}
    elseif(isset($e[($v = substr($v, 1))]) && !in_array('+'. $v, $m)){unset($e[$v]);}
   }
  }
 }
}
...
$C['any_custom_element'] = (!isset($C['any_custom_element']) or !empty($C['any_custom_element'])) ? 1 : 0;
...
$cF = array('a'=>1, 'article'=>1, 'aside'=>1, 'audio'=>1, 'button'=>1, 'canvas'=>1, 'del'=>1, 'details'=>1, 'dialog'=>1, 'div'=>1, 'dd'=>1, 'fieldset'=>1, 'figure'=>1, 'footer'=>1, 'header'=>1, 'iframe'=>1, 'ins'=>1, 'li'=>1, 'main'=>1, 'menu'=>1, 'nav'=>1, 'noscript'=>1, 'object'=>1, 'section'=>1, 'slot'=>1, 'style'=>1, 'td'=>1, 'template'=>1, 'th'=>1, 'video'=>1); // Flow; later context-wise dynamic move of ins & del to $cI
...
$eB = array('a'=>1, 'address'=>1, 'article'=>1, 'aside'=>1, 'blockquote'=>1, 'center'=>1, 'del'=>1, 'details'=>1, 'dialog'=>1, 'dir'=>1, 'dl'=>1, 'div'=>1, 'fieldset'=>1, 'figure'=>1, 'footer'=>1, 'form'=>1, 'ins'=>1, 'h1'=>1, 'h2'=>1, 'h3'=>1, 'h4'=>1, 'h5'=>1, 'h6'=>1, 'header'=>1, 'hr'=>1, 'isindex'=>1, 'main'=>1, 'menu'=>1, 'nav'=>1, 'noscript'=>1, 'ol'=>1, 'p'=>1, 'pre'=>1, 'section'=>1, 'slot'=>1, 'style'=>1, 'table'=>1, 'template'=>1, 'ul'=>1);
...
if(strpos($in, '-')){$inOk = array('*' => 1, '#pcdata' =>1);} // custom ele
...
for($i=-1, $ci=count($t); ++$i<$ci;){
 ...
 if($ql = count($q)){
  ...
  if(strpos($p, '-')){$ok = array('*'=>1, '#pcdata'=>1);}
 ...
 // get markup
 if(!preg_match('`^(/?)([a-z][^ >]*)([^>]*)>(.*)`sm', $t[$i], $r)){$x = $t[$i]; continue;}
 ...
 if(strpos($e, '-')){$ok[$e] = 1;}
 if((($ql && isset($cB[$p])) or (isset($cB[$in]) && !$ql)) && !isset($eB[$e]) && !isset($ok[$e]) && !isset($ok['*'])){
 ...
 for($k=-1, $kc=count($q); ++$k<$kc;){
  ...
  if(!isset($ok2[$e]) && !strpos($e, '-')){
   if(!$k && !isset($inOk[$e]) && !isset($inOk['*'])){continue 2;}
   $add = "</{$d}>";
...
$e = strtolower($m[2]);
static $eIC = array('annotation-xml'=>1, 'color-profile'=>1, 'font-face'=>1, 'font-face-src'=>1, 'font-face-uri'=>1, 'font-face-format'=>1, 'font-face-name'=>1, 'missing-glyph'=>1); // Illegal cust ele
if((!strpos($e, '-') && !isset($C['elements'][$e])) or (strpos($e, '-') && (isset($C['elements']['-' . $e]) or (!$C['any_custom_element'] && !isset($C['elements'][$e])) or isset($eIC[$e]) or preg_match('`[^-._0-9a-z\xb7\xc0-\xd6\xd8-\xf6\xf8-\x{2ff}\x{370}-\x{37d}\x{37f}-\x{1fff}\x{200c}-\x{200d}\x{2070}-\x{218f}\x{2c00}-\x{2fef}\x{3001}-\x{d7ff}\x{f900}-\x{fdcf}\x{fdf0}-\x{fffd}\x{10000}-\x{effff}]`u', $e)))){
...
static $aNU = array('accesskey'=>1, 'autocapitalize'=>1, 'autofocus'=>1, 'class'=>1, 'contenteditable'=>1, 'contextmenu'=>1, 'dir'=>1, 'draggable'=>1, 'dropzone'=>1, 'enterkeyhint'=>1, 'hidden'=>1, 'id'=>1, 'inert'=>1, 'inputmode'=>1, 'is'=>1, 'itemid'=>1, 'itemprop'=>1, 'itemref'=>1, 'itemscope'=>1, 'itemtype'=>1, 'lang'=>1, 'nonce'=>1, 'role'=>1, 'slot'=>1, 'spellcheck'=>1, 'style'=>1, 'tabindex'=>1, 'title'=>1, 'translate'=>1, 'xmlns'=>1, 'xml:base'=>1, 'xml:lang'=>1, 'xml:space'=>1); // Univ; excludes on*, aria*
...
foreach($aA as $k=>$v){
 if(((isset($d['*']) ? isset($d[$k]) : !isset($d[$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or (isset($aNO[$k]) && !isset($d['on*'])) or (isset($aNA[$k]) && !isset($d['aria*'])) or (!isset($d['data*']) && preg_match('`data-((?!xml)[^:]+$)`', $k)) or strpos($e, '-')) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])){
  if(isset($aNE[$k])){$v = $k;}
